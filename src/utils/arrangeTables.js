...
/**
 * Compute and assign x/y positions for every table in a diagram to produce a readable layout.
 *
 * Positions are written directly to each table object's `x` and `y` properties. When relationships are present,
 * tables are organized into layered rows using graph topology to reduce edge crossings; when no relationships exist,
 * a simple two-row fallback layout is used.
 *
 * @param {Object} diagram - Diagram data to layout.
 * @param {Array<Object>} [diagram.tables] - Array of table objects; each table should have an `id`, `fields` array, and will receive `x` and `y` coordinates.
 * @param {Array<Object>} [diagram.relationships] - Array of relationship objects with `startTableId` and `endTableId` linking tables.
 */
